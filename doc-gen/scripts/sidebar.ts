...
if (searchField){
	...
	function testSearch(searchString: string): void{
		...
		document.querySelectorAll('#sidebar .category').forEach((category: HTMLElement) => {
			category.style.display = Array.from(category.querySelectorAll('.instance')).every((el:HTMLElement) => {
