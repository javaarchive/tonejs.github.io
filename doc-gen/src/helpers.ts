...
export async function registerHelpers(allData: any){
	...
	registerHelper('markdown', function(content){
		...
		text = text.replace(brackets, (match: string, content: string): string => {
	 ...
	 */
	registerHelper('ifCond', function (v1: any, operator: any, v2: any, options: any) {
		switch (operator) {
			case '==':
	 ...
	 */
	registerHelper('toc', () => {
		return allData.groups.filter(group => {
