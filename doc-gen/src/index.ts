...
async function getInterfaceTemplate(): Promise<TemplateDelegate>{
	const iTempl = await readFile(resolve(TEMPLATE_DIR, 'interface.hbs'), 'utf-8')
...
async function getTypeTemplate(): Promise<TemplateDelegate>{
	const tTemplate = await readFile(resolve(TEMPLATE_DIR, 'typeTemplate.hbs'), 'utf-8')
...
async function getFunctionTemplate(): Promise<TemplateDelegate>{
	const fTemplate = await readFile(resolve(TEMPLATE_DIR, 'function.hbs'), 'utf-8')
...
async function main(){
	...
	await Promise.all(json.children.slice(0).map(async (docs) => {
		if (docs.name && docs.kindString === 'Class'){
			await writeFile(resolve(CLASS_DIR, `${docs.name}.html`), classTemplate(docs))
		} else if (docs.kindString === 'Interface'){
			await writeFile(resolve(INTERFACE_DIR, `${docs.name}.html`), interfaceTemplate(docs))
		} else if (docs.kindString === 'Type alias'){
			await writeFile(resolve(TYPE_DIR, `${docs.name}.html`), typeTemplate(docs))
		} else if (docs.kindString === 'Function'){
			if (docs.name === "Offline"){
				await writeFile(resolve(OUT_DIR, 'class.json'), JSON.stringify(docs, undefined, '\t'))
			}
